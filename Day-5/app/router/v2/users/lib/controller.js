...
class Controller {
  loginUser(req, res) {
    try {
      console.log(aUsers[req.nUserIndex].sRole);
      return res.status(status.statusSuccess).json({ sToken: jwt.sign({ sUserName: req.body.sUserName, sRole: aUsers[req.nUserIndex].sRole }, process.env.SECURITY_KEY, { expiresIn: '2h' }) });
    } catch (error) {
      console.error(error);
